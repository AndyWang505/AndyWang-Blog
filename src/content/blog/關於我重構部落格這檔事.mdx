...
在我開始接觸 Astro 時，過去曾經使用過 Jekyll 來自架部落格
...
不過 Astro 社群在中文圈的資訊比較少，碰到問題大部分都是查官方文件和透過 Github Issues 看人家類似有沒有碰過類似的問題來解決

## 設計理念

Astro 強調他的設計理念是打造 **以內容為中心的網站**，像是部落格、電商、行銷網站，而這些都有一個共通點，就是需要良好的 SEO 和速度

而 Astro 也不太需要花額外的時間成本去學習其他語言，因為是靜態生成，主體圍繞在 HTML、CSS、JavaScript，當然也可以用 TypeScript
...
這也是為什麼我在 Astro 寫不了 React hook 的原因，所以就不能用 SPA 那套思路來撰寫，應該去想這些東西是運行在 Server Side 的而不是 Client Side

關於 Astro 的設計理念可以參考 [Astro 官方網站](https://docs.astro.build/zh-tw/concepts/why-astro/#_top)，有詳細的說明，還滿有趣的～

## 最後新手開發上要注意的事

開發上前面有講到，就是思路需要調整，那基本上就沒有太大的問題了

同時你也能夠去插入 `<script>`，幫助你在客戶端運行某些功能～
