...
於是前人就開始思考，是否能在同一台伺服器為不同的應用程式提供「虛擬隔離」，以提高資源使用率，最終就催生了「虛擬化技術」。
...
而 Kubernetes 不僅是一個容器管理工具，最關鍵的是 **微服務架構的管理系統**，讓它能夠在多台機器上自動化部署、管理、擴展和監控容器，簡單的說，Kubernetes 透過更高層次的抽象來簡化容器運行與管理的方式。
...
另外，Kubernetes 提供多種 Zero Downtime Deployment 的策略來確保應用更新時不影響用戶體驗，其中最常見的兩種方式是 Blue-Green Deployment 和 Rolling Update Deployment。
